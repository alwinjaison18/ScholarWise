...
// Save a scholarship
router.post('/save-scholarship/:scholarshipId', authenticateToken, async (req, res) => {
  try {
...
router.delete('/save-scholarship/:scholarshipId', authenticateToken, async (req, res) => {
  try {
    const { scholarshipId } = req.params;
    const userId = req.user._id;
...
router.post('/apply-scholarship/:scholarshipId', authenticateToken, async (req, res) => {
  try {
    const { scholarshipId } = req.params;
...
router.put('/applied-scholarship/:scholarshipId', authenticateToken, async (req, res) => {
  try {
    const { scholarshipId } = req.params;
...
router.get('/search', optionalAuth, async (req, res) => {
  try {
    ...
    if (req.user) {
      const user = await User.findById(req.user._id)
        .select('savedScholarships appliedScholarships');
      
      userSavedIds = user.savedScholarships.map(id => id.toString());
      userAppliedIds = user.appliedScholarships.map(app => app.scholarshipId.toString());
    }
    ...
    res.json({
      ...
      data: {
        ...
        stats: {
          ...
          pendingApplications: user.appliedScholarships.filter(app => 
